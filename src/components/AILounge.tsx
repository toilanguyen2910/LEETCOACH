...
export default function AILounge() {
  ...
  return (
    <div className="bg-brand-panel border border-brand-border flex flex-col h-[650px] shadow-[4px_4px_0_#000] rounded-none text-[#E0E0E0]" id="ai-lounge-room">
      {/* Lounge Header */}
      <div className="px-6 py-4 border-b border-brand-border bg-brand-dark flex items-center justify-between shrink-0">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-xs font-black text-white flex items-center gap-1.5 uppercase tracking-widest">
              Góc AI Mentor Luyện Giải Thuật
              <Sparkles className="w-4 h-4 text-brand-neon animate-pulse" />
            </h2>
            <p className="text-[10px] text-zinc-500 font-mono uppercase mt-0.5">// Logic, Complexity analysis, Big Tech Coding Interviews</p>
          </div>
        </div>

        <div className="text-[9.5px] font-mono text-zinc-400 font-bold bg-[#111] px-2.5 py-0.5 rounded-none border border-brand-border">
          MODEL: GEMINI-2.5-FLASH
        </div>
      </div>

      {/* Messages Feed */}
      <div className="flex-1 overflow-y-auto p-6 space-y-5 bg-[#050505]" id="chat-scroller">
          ...
          return (
            ...
            >
              ...
              <div className={`w-8 h-8 rounded-none border flex items-center justify-center shrink-0 ${
                isAI ? 'bg-brand-panel border-brand-neon text-brand-neon' : 'bg-brand-dark border-brand-border text-white'
              }`}>
                {isAI ? <Bot className="w-4 h-4" /> : <User className="w-4 h-4" />}
              </div>

              {/* Message bubble */}
              <div className={`p-4 rounded-none border whitespace-pre-wrap leading-relaxed shadow-[2px_2px_0_#000] font-mono text-[11.5px] ${
                isAI 
                  ? 'bg-brand-panel border-brand-border text-zinc-300' 
                  : 'bg-brand-dark border-brand-neon/30 text-white'
              }`}>
                {msg.content}
              </div>
            </div>
        ...
        {loading && (
          <div className="flex gap-3 text-xs max-w-4xl">
            ...
            <div className="p-4 rounded-none bg-brand-panel border border-brand-border text-zinc-400 shadow-[2px_2px_0_#111] flex items-center gap-2 select-none font-mono">
      ...
      <div className="px-6 py-3 bg-brand-dark border-t border-brand-border overflow-x-auto whitespace-nowrap flex gap-2 shrink-0 scrollbar-none select-none">
        {presetQuestions.map((q, i) => (
          <button
            ...
            disabled={loading}
            className="text-[10px] font-mono font-bold uppercase tracking-wider bg-brand-panel hover:bg-brand-border border border-brand-border text-zinc-300 hover:text-brand-neon px-3 py-1.5 rounded-none transition-colors cursor-pointer disabled:opacity-50"
          >
            {q.title}
          </button>
        ))}
      ...
      <div className="p-4 border-t border-brand-border bg-brand-dark shrink-0">
        ...
        >
          <input
            ...
            className="flex-1 text-xs px-4 py-3 border border-brand-border rounded-none focus:outline-none focus:border-brand-neon bg-brand-panel text-white placeholder-zinc-750 font-mono"
          />
          <button
            type="submit"
