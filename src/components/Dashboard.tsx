...
export default function Dashboard({ problems, activityLogs, streakDays, maxStreak, onSelectProblem }: DashboardProps) {
  ...
  return (
    <div className="space-y-6" id="dashboard-tab">
      {/* Welcome Banner */}
      <div className="bg-brand-panel text-brand-text rounded-none p-6 border border-brand-border relative overflow-hidden shadow-[4px_4px_0px_var(--color-brand-border-val)]">
        <div className="absolute top-0 right-0 p-4 font-mono text-[90px] font-black leading-none text-brand-border-light/10 select-none pointer-events-none">
          {problems.length < 10 ? `0${problems.length}` : problems.length}
        </div>
        <div className="max-w-2xl relative z-10">
          <span className="bg-brand-neon text-brand-neon-text font-black uppercase px-2.5 py-1 text-[9px] tracking-widest">
            ACTIVE SESSION
          </span>
          <h2 className="text-2xl font-black text-brand-text mt-4 uppercase italic tracking-tight">
            Chào mừng bạn quay lại hệ thống ôn luyện LeetCode!
          </h2>
          <p className="text-brand-text-muted mt-2.5 text-xs font-light leading-relaxed">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="col-span-1 lg:col-span-2 space-y-6">
          ...
          {/* Core Tips Card */}
          <div className="bg-brand-panel border border-brand-border rounded-none p-6 shadow-[3px_3px_0_var(--color-brand-border-val)]">
            ...
            <div className="space-y-4">
              {optimizationTips.map((tip, idx) => (
                <div key={idx} className="flex gap-3 text-xs leading-relaxed">
                  ...
                  <div>
                    <h4 className="font-bold text-brand-text text-xs">{tip.title}</h4>
                    <p className="text-brand-text-muted text-[11px] mt-1 leading-relaxed border-l-2 border-brand-neon/20 pl-2">
                      {tip.text}
                    </p>
                  </div>
                </div>
        ...
        <div className="space-y-6">
          {/* Daily 추천 Challenge */}
          {recommendation && (
            <div className="bg-brand-panel border border-brand-border p-5 rounded-none shadow-[3px_3px_0_var(--color-brand-border-val)]" id="daily-recommendation">
              <div className="flex justify-between items-start mb-4">
                <span className="text-[10px] bg-brand-neon text-brand-neon-text font-extrabold px-2.5 py-0.5 uppercase tracking-wider">
                  Đề xuất hôm nay
                </span>
              ...
              <h4 className="font-extrabold text-brand-text text-base tracking-tight mb-2 uppercase italic">
                {recommendation.title}
              </h4>
              <p className="text-[11px] text-brand-text-muted line-clamp-3 leading-relaxed mb-4">
                {recommendation.description.replace(/```[\s\S]*?```/g, '').replace(/[\#*`]/g, '')}
              </p>

              <div className="flex flex-wrap gap-1.5 mb-4">
                {recommendation.tags?.slice(0, 3).map((tag, i) => (
                  <span key={i} className="text-[9px] font-mono bg-brand-dark text-brand-text-muted/60 px-2 py-0.5 border border-brand-border">
                    //{tag}
                  </span>
                ))}
              </div>
              ...
              <button
                ...
                className="w-full bg-brand-neon hover:bg-brand-text text-brand-neon-text hover:text-brand-panel font-black uppercase text-xs px-4 py-3 rounded-none flex items-center justify-center gap-2 transition-colors cursor-pointer border border-brand-border shadow-[2px_2px_0_var(--color-brand-border-val)]"
              >
                <Play className="w-3.5 h-3.5 text-brand-neon-text fill-brand-neon-text hover:fill-brand-panel group-hover:text-brand-panel" />
          ...
          {/* Recent History log */}
          <div className="bg-brand-panel border border-brand-border p-5 rounded-none shadow-[3px_3px_0_var(--color-brand-border-val)]">
            ...
            {recentSolved.length > 0 ? (
                  ...
                  >
                    <div className="flex justify-between items-start gap-2">
                      <button 
                        ...
                        className="font-bold text-brand-text hover:text-brand-neon hover:underline text-left truncate leading-tight uppercase cursor-pointer"
                      >
                        {prob.title}
                      </button>
                    ...
                    <div className="flex items-center gap-1 text-[9px] text-brand-text-muted/65 font-mono">
                      <CheckCircle2 className="w-3.5 h-3.5 text-emerald-400 shrink-0" />
                      <span>Đã giải: {new Date(prob.lastSolvedAt!).toLocaleDateString('vi', {day: 'numeric', month: 'short'})}</span>
                    </div>

                    {prob.customNotes && (
                      <p className="text-[10px] text-brand-text-muted italic mt-0.5 line-clamp-1 border-l border-brand-neon/30 pl-1.5">
                        "{prob.customNotes}"
                      </p>
                    )}
                  </div>
              ...
              <div className="py-8 text-center text-brand-text-muted italic bg-brand-dark border border-brand-border text-xs">
                <p>Chưa có bài giải gần đây.</p>
                <p className="text-[10px] mt-1 text-brand-text-muted/60">// Thử ngay với LeetCoach</p>
              </div>
            )}
          </div>
        </div>
